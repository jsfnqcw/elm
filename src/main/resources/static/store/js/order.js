...
function bodyload() {
  var url = "http://" + window.location.host + "/store/getOrders"
  ...
  var url = "http://" + window.location.host + "/store/getOrderUser"
  $.post(url, function(result) {
    if (result == null) {return;}
    var ResTypes = JSON.parse(result);

    var htmlSel = "<option value='0' selected=''>筛选用户</option>";
...
function select(){
  ...
  var url = "http://" + window.location.host + "/store/getSelectedOrders"
  var message = {
    "time" : sel1,
    "price" : sel2,
    "user" : sel3
